...
while True:
    ...
    if len(idxs) > 0:
        ...
        for i in idf:
            ...
            center.append(cen)
            cv2.circle(frame, tuple(cen),1,(255,0,0),1)
        ...
        if (c%timeF != 0):
            list1.append(high_risk_p)
            list3.append(low_risk_p)
        else:
            list2.append(sum(list1)/len(list1))  
            list4.append(sum(list3)/len(list3))
            if sum(list1)/len(list1)>6:
            	print('alert','{}*10s'.format(len(list2)))
            list1=[]
            list3=[]
        c = c+1
        cv2.waitKey(1)
        ...
        for i in idf:
            ...
            cv2.putText(FR, "CUSP Social Distancing Analyser. COVID-19", (210, H+60),
...
print('high risk people',list2, len(list2))
print('low risk people',list4,len(list4))
writer.release()
vs.release()
